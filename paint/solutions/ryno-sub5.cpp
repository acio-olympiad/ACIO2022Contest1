...
void fracturingSearch() {
    ...
    for (ll rep = 0; rep < K - 1; ++rep) {
        ...
        for (ll d = curr.hasChoice; d <= D; ++d) {
            ...
            for (ll itemI = curr.used[d] + 1; itemI < (ll)costs[d].size(); ++itemI) {
                State neighbour = curr;
                neighbour.cost += costs[d][itemI] - costs[d][curr.used[d]];
            ...
            if (curr.used[d] != (ll)costs[d].size()) {
                State neighbour = curr;
                neighbour.cost -= costs[d][curr.used[d]];
                neighbour.numItems--;
                neighbour.hasChoice = d + 1;
